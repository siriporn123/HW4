...
function init() {
	...
	renderer.gammaOutput = true;
	renderer.setSize(canvasWidth, canvasHeight);
	renderer.setClearColorHex( 0x75FFCA, 1.0 );
...
try {
  init();
  showGrids();
  var geo = PolygonGeometry(8, new THREE.Vector3( 3, 4, 0 ));
  var material = new THREE.MeshBasicMaterial( { color: 0x14148C, side: THREE.FrontSide } );
  var mesh = new THREE.Mesh( geo, material );
  scene.add( mesh );
  addToDOM();
  render();
